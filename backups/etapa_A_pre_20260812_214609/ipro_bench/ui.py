...
from .test_manager import BenchTest, TestManager
...
class StatusCard(QFrame):
    def __init__(self, title: str, value: str, detail: str) -> None:
        ...
        title_label = QLabel(title.upper(), objectName="cardTitle")
...
class MainWindow(QMainWindow):
    NAVIGATION = ("Dashboard", "Supervisão", "I/O", "Modbus", "Mapa", "Testes", "Evidências", "Diagnóstico", "Relatórios")
    ...
    def _build(self) -> None:
        ...
        side_layout.addWidget(QLabel("v0.1.0 · ETAPA 1", objectName="subtitle"))
        ...
        self.pages.addWidget(self._placeholder("I/O", "Entradas e saídas aguardam o mapa Modbus oficial."))
        ...
        self.pages.addWidget(self._placeholder("Relatórios", "Exportação PDF/CSV/JSON será implementada na próxima etapa."))
    ...
    def _supervision(self) -> QWidget:
        page, layout = self._page()
        layout.addWidget(QLabel("Variáveis prioritárias · mapa ativo ainda não disponível", objectName="title"))
    ...
    def _map_page(self) -> QWidget:
        ...
        layout.addWidget(QLabel(f"{payload['metadata']['status']} · {validation.variable_count} variáveis · SHA-256 {validation.sha256[:16]}…", objectName="muted"))
    ...
    def _tests_page(self) -> QWidget:
        ...
        self.tests_table.setHorizontalHeaderLabels(("ID", "TESTE", "CATEGORIA", "RESULTADO", "CRIADO EM"))
    ...
    def _evidence_page(self) -> QWidget:
        ...
        self.evidence_table = QTableWidget(7, 2)
        self.evidence_table.setHorizontalHeaderLabels(("CATEGORIA", "REGISTROS"))
        for row, name in enumerate(("tcp", "rs485", "testes", "sensores", "alarmes", "cenarios", "relatorios")):
    ...
    def _create_test(self) -> None:
        ...
        for col, value in enumerate((test.id, test.name, test.category, test.result.value, test.created_at)):
