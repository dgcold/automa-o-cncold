...
from .explainable_diagnostics import ConclusionState, DiagnosticRepository, ExplainableDiagnosticEngine, RuleCatalog
...
from .incident_analysis import IncidentAnalyzer, WindowPreset
from .operational_health import HealthRepository, OperationalHealthEngine, PeriodKind
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, project_root: Path) -> None:
        ...
        self.defrost_analyzer = DefrostCycleAnalyzer(self.blackbox_store, self.baseline_repository)
        self.incident_analyzer = IncidentAnalyzer(self.blackbox_store, self.baseline_service)
        self.diagnostic_repository = DiagnosticRepository(project_root / "dados" / "diagnosticos.sqlite3")
        self.rule_catalog = RuleCatalog(project_root / "config" / "diagnostic_rules.json")
        self.diagnostic_engine = ExplainableDiagnosticEngine(self.blackbox_store, self.diagnostic_repository, self.rule_catalog.load())
        self.anomaly_repository = AnomalyRepository(project_root / "dados" / "anomalias.sqlite3")
        self.anomaly_engine = AnomalyEngine(self.blackbox_store, self.baseline_repository, self.anomaly_repository)
        self.health_repository = HealthRepository(project_root / "dados" / "saude.sqlite3")
        self.health_engine = OperationalHealthEngine(self.blackbox_store, self.anomaly_repository, self.defrost_analyzer, self.health_repository)
