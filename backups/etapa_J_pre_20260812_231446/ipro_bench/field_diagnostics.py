...
class BlackBoxRecorder:
    ...
    def communication(self, connected: bool, source: str, detail: str = "") -> None:
        session_id = self._require_active()
    ...
    def alarm(self, alarm_id: str, active: bool, message: str, severity: str = "ALARME") -> None:
        timestamp, timestamp_ns = high_resolution_timestamp()
    ...
    def marker(self, label: str, notes: str = "", kind: TimelineKind = TimelineKind.MARKER) -> None:
        timestamp, timestamp_ns = high_resolution_timestamp()
        self.store.append(self._require_active(), TimelineRecord(
            kind, timestamp, timestamp_ns, name=label, message=notes or label,
        ))

    def deviation(self, variable_id: str, message: str, evidence: dict[str, Any] | None = None) -> None:
        timestamp, timestamp_ns = high_resolution_timestamp()
        self.store.append(self._require_active(), TimelineRecord(
    ...
    def recovery(self, message: str, evidence: dict[str, Any] | None = None) -> None:
        timestamp, timestamp_ns = high_resolution_timestamp()
        self.store.append(self._require_active(), TimelineRecord(
