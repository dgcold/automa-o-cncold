...
from .anomaly_analysis import AnalysisState, AnomalyRepository
...
class OperationalHealthEngine:
    ...
    def _trends(self,ids,anomalies):
        ...
        recovery=[fmean([(r["timestamp_ns"]-prev["timestamp_ns"])/1e9 for prev,r in pairwise(rows) if r["kind"]==TimelineKind.RECOVERY.value]) if any(r["kind"]==TimelineKind.RECOVERY.value for r in rows) else 0 for sid in ids for rows in [self.blackbox.query(sid)]]
