...
class IncidentAnalyzer:
    ...
    def intermittent_failures(self, session_ids: Iterable[str], minimum_occurrences: int = 2) -> list[IntermittentFailure]:
        ...
        for key,items in groups.items():
            ...
            intervals=tuple((b[1]["timestamp_ns"]-a[1]["timestamp_ns"])/1e9 for a,b in pairwise(ordered))
            result.append(IntermittentFailure(key,len(items),len({s for s,_ in items}),ordered[0][1]["timestamp"],ordered[-1][1]["timestamp"],intervals,tuple(e["id"] for _,e in ordered)))
        return result
